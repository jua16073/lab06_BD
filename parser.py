...
def parseXml(f):
    ...
    #print(" " + str(dom.nodeType))
    #print(dom.toprettyxml())
    #print(""+ str(dom))
    e=(getElementsByTagNameNR(dom, 'Items'))
    p=(getElementsByTagNameNR(e[0],'Item'))
    n=(getElementsByTagNameNR(p[0],'Name'))
    #print getElementText(n)
    print len(p)
    x=0
    y=0
    m=20
    ...
    while i<len(p):
        ...
        i+=1
    while x<len(p):
        datos[x][0]=getElementByTagNameNR(p[x],'Name')
        datos[x][1]=getElementByTagNameNR(p[x],'Category')
        datos[x][2]=getElementByTagNameNR(p[x],'Currently')
        datos[x][3]=getElementByTagNameNR(p[x],'First_Bid')
        datos[x][4]=getElementByTagNameNR(p[x],'Number_of_Bids')
        x+=1
    print getElementText(datos[0][0])
